...
They provided you with “orders” database and seek answers to the following queries as the results 
from these queries will help the company in making data-driven decisions
...
/*Q7. Write a query to display 
carton id, (len*width*height) as carton_vol and 
identify the optimum carton (carton with the least volume whose volume is greater than the total volume
of all items (len * width * height * product_quantity)) for a given order whose order id is
10006, Assume all items of an order are packed into one single carton (box).

(1 ROW) [NOTE: CARTON TABLE,PRODUCT TABLE] */

select * from carton;
select * from product;

SELECT  C.CARTON_ID AS "carton id",
(C.LEN*C.WIDTH*C.HEIGHT) AS "carton_vol" -- (len*width*height) as carton_vol
FROM ORDERS.CARTON C
WHERE (C.LEN*C.WIDTH*C.HEIGHT) >= (
-- subquery to take the volume of details from order_items and product tables.
SELECT 
SUM(P.LEN*P.WIDTH*P.HEIGHT*OI.PRODUCT_QUANTITY) AS VOL  -- optimum carton value
FROM ORDERS.ORDER_ITEMS OI
INNER JOIN ORDERS.PRODUCT P ON
OI.PRODUCT_ID = P.PRODUCT_ID
WHERE OI.ORDER_ID = 10006) -- for a given order whose order id is 10006.
ORDER BY (C.LEN*C.WIDTH*C.HEIGHT) ASC
LIMIT 1;

/* Q8. Write a query to display details 
(customer id,customer fullname,order id,product quantity) of customers 
...
WHERE OH.ORDER_STATUS = 'Shipped' AND (PAYMENT_MODE = "Net Banking" OR PAYMENT_MODE = "Credit Card")
GROUP BY OH.ORDER_ID
HAVING Total_Order_Quantity > 10 
ORDER BY CUSTOMER_ID;

/* 9. Write a query to display the 
order_id, customer id and customer full name of customers
starting with the alphabet “A” along with (product_quantity) as total quantity of products
shipped for order ids > 10030. (5 ROWS)
...
WHERE CUSTOMER_FNAME like 'A%' AND OH.ORDER_ID>10030 AND OH.ORDER_STATUS = "Shipped" 
GROUP BY OH.ORDER_ID
ORDER BY Customer_FullName;

/* 10. Write a query to display 
product class description ,total quantity (sum(product_quantity),
Total value (product_quantity * product price) 
